    ...
    ! Vesrion 1.003: March 23, 2024
    !****************************************************************************
    program Autovar
    use sortem ! Quicksort module
    use gen_subs! for general subroutines in modules: histogram
    use spacing ! To calculate horizontal and vertical space.
    use stratco ! module to define strat. transformation of Z coordinates.
    use alldir ! omnidirectional calculation of pairs
    use moi_aux ! MOI algorithm
    use sp3d ! Module to calculate 3D spacing without considering DHID
    ...
    !   Geolocated process(to be assigned in the next stages of the code):
    ...
    real (kind=8) :: xmin,xmax,ymin,ymax,zmin,zmax,av,ss
    real:: var(512),vrt
...
1   read(lin, '(a4)' , end=98) str(1:4)
    ...
    ! Check if Z column is provided, X and Y should always be provided
    if (xcol.eq.0) then
    ...
    ! Check if ID column is provided, this is neccesary for spacing calculation
    if (idcol.eq.0) then
        not_dhid = .true.
        if (.not.not_z) then
    ...
    if (geolocvar .eq. 0) then
        ...
        !fmt = '(I2.1)' ! an integer of width 5 with zeros at the left
        outfl_mod = outfl_mod(:index(outfl_mod, '.')-1) ! Extract characs before the '.out'
        write(*,*) ' There will be as many models as anchors = ',nanchor
        do i = 1, nanchor
            if (i .le. 9) fmt = '(I1.1)' ! an integer of width 5 with zeros at the left
            if (i .gt. 9) fmt = '(I2.1)'
            write (ii,fmt) i
    ...
    if (geolocvar .eq. 0) then
        ...
        outfl_exp = outfl_exp(:index(outfl_exp, '.')-1) ! Extract characs before the '.out'
    ...
    if (geolocvar .eq. 0) then
        ...
        dbgfl = dbgfl(:index(dbgfl, '.')-1) ! Extract characs before the '.out'
    ...
    if (geolocvar .eq. 0) then
        ...
        !fmt = '(I2.2)' ! an integer of width 5 with zeros at the left
        varmodgs = varmodgs(:index(varmodgs, '.')-1) ! Extract characs before the '.out'
    ...
    if (geolocvar .eq. 0) then
        ...
        !fmt = '(I2.2)' ! an integer of width 5 with zeros at the left
        wtsfl = wtsfl(:index(wtsfl, '.')-1) ! Extract characs before the '.'
...
3   close(lin)
    ...
    vv = v
    iidd = id
    !Until this point not considering declustering weights, if
    !They could be provided for the user. This program will not consider it.
    allocate(wt(nd))
    wt=1
    !
...
5       close(lin)
    ...
    if (geolocvar .eq. 0) then  ! not using geolocated option
        ...
        do i=1,nanchor
            do j= 1,nd
                ...
                ! wt(i) could be considered for future inclusion of
                ! declustering weights, at this point is just a constant.
    ...
    if (dat3d) then
        if  (.not.not_dhid) then
            ...
            ! if there is no dhid information, this calculations will be skipped
            ! however there is high chance of wrong id of directions
        else
            continue
        end if
    end if
...
111 do anx = 1, nanchor
        ...
        !
        if (ndir.eq.2) then
            max_dist = sqrt((maxval(xyz(:,1))-minval(xyz(:,1)))**2 + &
        ...
        if (ndir.eq.3) then
            ...
            if (geolocvar .eq. 1) then ! Select points and weights for each anchor
                nax = 0
                ! After testing many search radius(sr) for MOI, 0.3 gives the best results
                sr =0.3
                do i = 1, nd
                    ddaxx = abs((x(i)-anchx(anx)))
                    ddaxy = abs((y(i)-anchy(anx)))
            ...
            if (dep_type .eq. 1) then
                if (geolocvar .eq. 1) then  !Reevaluate the Z extends since it's
                    ...
                    nd_tmp = 0
                    ! make sure that the number of data is valid whenever is using
                    ! or not anchors
            ...
            if (dep_type .eq. 2) then
                call stratcot(xyz,v,nd,idcol,id,zavr,debug,lavr,xs,ys,zs,vs)
                ! Check rate between extension in the horizontal and vertical
                rt_dh = avgavg/lavr
                ! temporal conditionals to keep the horizontal datasets working right
                if (rt_dh .lt.10) then
                    max_dist = 1.5*sqrt((maxval(x)-minval(x))**2 + &
                        (maxval(y)-minval(y))**2 + &
                        (maxval(z)-minval(z))**2)
                ...
                if (rt_dh .ge.10) then
                    strattrans = 1
                    write(*,*) ' Stratigraphic transformation done using proportional method '
                    allocate(xyzs(nd,3))
                    xyzs(:,1) = xs
                    xyzs(:,2) = ys
                    xyzs(:,3) = zs
                    scaling = .true.
                    max_dist = 1.5*sqrt((maxval(xyzs(:,1))-minval(xyzs(:,1)))**2 + &
                        (maxval(xyzs(:,2))-minval(xyzs(:,2)))**2 + &
                        (maxval(xyzs(:,3))-minval(xyzs(:,3)))**2)
                    call omnipair(nd,max_dist,tmin,tmax,xyzs(:,1),xyzs(:,2),xyzs(:,3),v,&
                        geolocvar,wtscurrent,outputidx,sqdfdist,dx,dy,dz,sq_df,idf,wtsomni,debug)
                end if
            end if
        ...
        ! there is no need for reordering when strat coord are being used
        if (ndir .eq. 3 .and. strattrans .eq.1) then
            eigveco(1,1) = eigvec(1,2)! xmaj
            eigveco(2,1) = eigvec(2,2)! ymaj
            eigveco(3,1) = eigvec(3,2)! zmaj
            eigveco(1,2) = eigvec(1,1)! xmin
            eigveco(2,2) = eigvec(2,1)! ymin
            eigveco(3,2) = eigvec(3,1)! zmin
            eigveco(1,3) = eigvec(1,3)! xter
            eigveco(2,3) = eigvec(2,3)! yter
            eigveco(3,3) = eigvec(3,3)! zter
            if (debug) then
                write(*,*) ('Ordered Eigenvectors: ')
                do i=1,3
                    write(*,'(3f18.3)') (eigveco(i,k),k = 1,3)
        ...
        if (ndir .eq. 2) then
            ...
            eigveco(2,3) = eigvec(2,dblerotindexx(3))! yter
            eigveco(3,3) = 1! zter
            ! Get azimuth and dip atan(lrotmatrob(1,2)/lrotmatrob(1,1))
        ...
        ! Make sure that there is the right set of eigenvectors, according to GSLIB
        ! angle convention (this is centred on 3D analysis)
        ...
        ! If the strat transformation has been used, set the x,y, and z coordinates
        ! to the zstrat coord
        if (strattrans.eq.1) then
            x = xyzs(:,1)
            y = xyzs(:,2)
            z = xyzs(:,3)
            v = vs
            write(*,*) 'USING STRATIGRAPHIC COORDINATES'
        end if
        ...
        do i=1,nd
            ...
            ! Need to save dhid and recalculate spacings in horz and vert.
            dmajco(i) = dmaj
            dminco(i) = dmin
            dterco(i) = dter
        ...
        if (ndir.eq.3) then
            !dmajlag = max(EPSLON,abs(majspc))
            !dminlag = max(EPSLON,abs(minspc))
        ...
        ! If the rotation matrix is not adequate this could fail.
        ...
        if (geolocvar .eq. 1)then
            ...
            !if (max_ter_dist .gt. 0.3*max_dist) max_ter_dist=0.3*max_dist
        ...
        !! Enforce order relations if needed
        !if (dmaj .lt. dmin) dmin = dmaj
        ...
        ! 4. Allocate the arrays according to the number of lags+1 (it could be
        ! necessary to consider close distances on the expvar subroutine).
        !major direction
        ...
        !
        ! temporary modification to test against python algorithms
        if (dep_type .eq.1) then !return to original disposition, no sortem!
            x = xxx
            y = yyy
            z = zzz
            v = vv
        ...
        !
        ! cc,aa1,aa2,aa3 arrays were not previously defined, they got default values,
        ! then new values are assigned to ensure the conditions(std var and long enough range)
        c0 = 0! this is the nugget effect, it is hardcoded now (could be set by the user)

        !all started cc values will be set to c0/3 since the calculations multiply them
        !by a random number, if the cc are zero from the beggining, there will be no
        !change, the other option is to set cc = cc + (random value)
        cc = 1
        silln = c0
        ...
        if (varimp .eq. 1) then
            ...
            if (optmajmin1 .lt. 1.1) then
                ...
                call varexpflx(x,y,z,v,lrotmat,nd,usemaj,usemin,useter,nmaj_lag,nmin_lag,nter_lag,&
                    ...
                    tlvternum,tlvtercov,tlvtermu,tlvtermuh,tlvtersu,tlvtersuh,tlvterdis,tlvterwt,&
                    anchx(anx),anchy(anx),anchz(anx),anx,geolocvar,strattrans)
                ! Calculate the update model, update the sizes of the arrays base
...
103     format('Variogram model points',/,'7',/, 'Variogram Index',/,&
        ...
        !write(i_exp)
        !104 format('Optimized variogram model')
...
105     format('Weights',/,'4',/,'X',/,'Y',/,'Z')
        ...
        if (strattrans .eq. 1) deallocate(xyzs)
...
13  format('-1.0e21   1.0e21                 ',&
        '-   trimming limits')
    !     write(lun,14)
    !14   format('1000  700  70                    ',&
    !           '-   Major, minor and tertiary ranges')
