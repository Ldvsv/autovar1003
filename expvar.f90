    ...
    ! code
    !
    !!! Weighting function will follow Gaussian kernel convention.
    fmt = '(I2.2)'
    write (ii,fmt) anx
    ...
    if (geoloc .eq. 1) then
        ...
        if (usemajev) then
            ...
            ! Calculate the declustered weights for each lag?
        end if
...
120 format(4x,4f12.4,2x,4f12.4)
    ...
    reftanh = tan(22d0*DEG2RAD)
    reftanv = tan(5d0*DEG2RAD)
    !Set same weights at this stage
    wmaj=1;wmin=1;wter=1

    if (geoloc .eq. 0) then
        ! If strat transformation has been done, another approach of restrictions
        ! will be set in place
        ! As testing just changing the calculation to the disseminated deposit type
        searchdif = .false.
        if (strattransev.eq.1) searchdif = .true.
        ...
        do i=1,ndev
            do j=1,ndev
                ...
                if(usemajev.and.i.ne.j) then
                    ...
                    !if (deptypeev.eq.1) dmajtolh = 4*tolfrac*dmajlagev
                    if (deptypeev.eq.1) dmajtolh = 2*tolfrac*dmaj
                    if (deptypeev.eq.1) dmajtolv = 4*zavranisev
                    !if (deptypeev.eq.2) dmajtolh = 10000*dmajlagev
...
220                     continue
                ...
                if(useminev.and.i.ne.j) then
                    ...
                    !if (deptypeev.eq.1) dmintolh = 4*tolfrac*dminlagev
...
221                     continue
                ...
                if(useterev.and.i.ne.j) then
                    ...
                    !if (deptypeev.eq.1) dtertolh = 2*tolfrac*dter
...
120 format(4x,4f12.4,2x,4f12.4)
    ...
    ! stage 1 is for total weight calculation and stage 2 is for getting
    ! all values for the correlogram (including the standardized weights)
    ...
    i_exp = 9
    nmaj =0
    ! Sort in function of X: it will improve speed
    !call dblemodsortem(xyzrot(:,1),ndev,3,xyzrot(:,2),xyzrot(:,3),dblerotindex)
    !
    ! Major direction
    !
    ...
    if (dir .eq. 1) then
        ...
        do i = 1, nd
            ...
            if (deptype.eq.1) then
                !farthest = xyz(i,1) + nmajlagev*lagsize+lagtol
            ...
            do j = 1, nd
                ...
                do l = 0, nlag
                    if ((h .ge. (l*lagsize-lagtol)) .and. &
                        ...
                        !write(i_exp,'(3f14.4,i7)') danx,wmaj,h,l+1
                        nmaj = nmaj+1
                    end if
                end do
    ...
    real(kind=8), parameter :: BIGDBLE=1d21,SMALLDBLE=1d-6
    ! inputs
    integer, intent(in) :: nd,deptype,nlag,dir,glc! glc for geolocated option
    ! stage 1 is for total weight calculation and stage 2 is for getting
    ! all values for the correlogram (including the standardized weights)
    real(kind = 8), dimension(nd,3), intent(in):: xyz
    real(kind = 8), dimension(nd), intent(in):: v
