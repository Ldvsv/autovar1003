    ...
    ! than 0.2m apart for the current tested datasets
    minspac = 1
    minspac2= minspac*minspac
    ! The max spacing will be calculated using the farsthest coordinates
    ...
    !midx = (maxval(xyzsh(:,1))+minval(xyzsh(:,1)))/2
    !midy = (maxval(xyzsh(:,2))-minval(xyzsh(:,2)))/2
    !drillhole
    ...
    if(idd==1) then
        ...
        idh = 1
        do i=2,ndsh
            ! to group coordinates by drillhole
            ! this works for vertical drillholes
            ! otherwise it would take the last coordinate
            ! and set as the coordinate of the drillhole
            ! however it could be set to get a 2d dataset
            ! of the collars.
    ...
    ! If there is only 2D dataset idh will go to 2 and
    ! xloc(1) and yloc(1) will be negative, this will produce
    ! unexpected errors on calculations of spacing, then a solution
    ! could be setting that distance manually: find the minimum values
    ...
    do  idat=1,ndt
        ...
        ! End Main Loop
        ! in some cases nb is 0 and this breaks the algorithm( could be related to clusters?)
        ! not sure, but a temporary solution will be just go to the next point
3       continue
    end do
    ...
    allocate(xdistdh(ndh)) ! to save the average distance spacing of each ddh
    allocate(ydistdh(ndh)) ! to save the average distance spacing of each ddh
    allocate(zdistdh(ndh)) ! to save the average distance spacing of each ddh
    allocate(lenght(ndh)) ! to save the lenght of each ddh
