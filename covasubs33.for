...
C%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
      ...
      subroutine cova33(x1,y1,z1,x2,y2,z2,ivarg,nst,MAXNST,c0,it,cc,aa,
...
c
c
c EXTERNAL REFERENCES: sqdist_    computes anisotropic squared distance
c                      rotmat_    computes rotation matrix for distance
c-----------------------------------------------------------------------
      parameter(PI=3.14159265359,PMX=1.e10,EPSLON=1.e-10)
      integer   nst,it(*)
      real      c0,cc(*),aa(*)
      real*8    rotmat(MAXROT,3,3),hsqd,sqdist_
...
c
      hsqd = sqdist_(x1,y1,z1,x2,y2,z2,irot,MAXROT,rotmat)
...
c
          if(ist.ne.1) then
              ...
              hsqd=sqdist_(x1,y1,z1,x2,y2,z2,ir,MAXROT,rotmat)
          end if
          h = real(dsqrt(hsqd))
c
...
c
      ...
      real*8 function sqdist_(x11,y11,z11,x22,y22,z22,ind,MAXROT,rotmat)
c-----------------------------------------------------------------------
c
...
c INPUT VARIABLES:
c
c   x11,y11,z11         Coordinates of first point
c   x22,y22,z22         Coordinates of second point
...
c
     ...
     +    + rotmat(ind,i,3) * dz
      ...
      subroutine setrot_(ang1,ang2,ang3,anis1,anis2,ind,MAXROT,rotmat)
c-----------------------------------------------------------------------
c
