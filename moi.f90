    ...
    !
    ! OUTPUT PARAMETERS:
    ! * Eigenvectors, eiagenvalues and MOI matrix
    !
    !
    ! INTERNAL VARIABLES
    ...
    !  ! Get distances from the middle
    !do i=1, outputidxmoi
    !    dist(i) = sqrt(dxmoi(i)**2+dymoi(i)**2+dzmoi(i)**2)
    !end do
    !! Select points inside one fifth of domain size
    !! The definition of the directions will be clearer at
    !! short scales.
    !
    !max_rad_s = 0.2*domsizemoi
    ...
    ! Just consider values between P75 and P99

    !do i=1, outputidxmoi
    !    if ((sqdfmoi(i).ge.min_corr).and.(sqdfmoi(i).le.max_corr)) then
    !          nd = nd+1
    !    else
    !        continue
    !    end if
    ...
    !
    ! The MOI will be directly calculated using the points instead
    ! of averages, dxmoi, dymoi and dzmoi can be negative for calculations,
    ! also the values off diagonal are negatives
    ! Ref: Classical mechanics by Greiner. P 205
    i=0
    j=0
    k=0
    ...
    ! The MOI tensor could also divide by the distance to avoid the big
    ! influence of distant points with high values.
    moimatmoi = 0
    power = 2.5

    do i=1,nd-1
        ! for elements off the diagonal (lower matrix)(signs according to MIT notes)
